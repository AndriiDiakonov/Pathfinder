...
r_mx *mx_floyd_warshall(int num_places, int matrix[num_places][num_places], char **places) {
    ...
    for (int a = 0; a < num_places; a++) {
        for (int i = 0; i < num_places; i++) {
            for (int j = 0; j < num_places; j++) {
                if (matrix[i][j] >= matrix[i][a] + matrix[a][j] && matrix[i][j] != 0 && matrix[a][j] != 0) {
                    matrix[i][j] = matrix[i][a] + matrix[a][j];
                    mx_push_route_front(&route, places[i], places[a], places[j], matrix[a][j], matrix[i][a], matrix[i][j]);
                }
            }
        }
    }
